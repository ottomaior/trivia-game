...
- **Effects modes:** `full` (everything), `lite` (the studio and camera, no particle canvas or ambient animation) and `flat` (the plain screens, no motion). The TV measures its frame rate for 3 seconds while the studio idles and again in the first reveal (the busiest moment), and steps down (full → lite → flat) if it can't keep up (under 40 fps, or well under the idle rate; a steady 30 fps counts as a capped display, not a slow one); the result is remembered. Force a mode with `/tv?fx=full|lite|flat` (also remembered); the old `?lowfx=1` means flat. The OS "reduce motion" setting also starts flat.
- **Screen shape and iPads:** the TV is laid out on a 16:9 canvas; a squarer screen (an iPad mirrored to the TV with AirPlay or an HDMI adapter) fits the canvas to its width, and the studio stands on the bottom edge with more backdrop above. The TV keeps its screen awake during a game (Screen Wake Lock) and asks Safari to play its sound as media, so the silent switch doesn't mute it (Safari 17+; on older iPads turn Silent Mode off). Keep the iPad in landscape and Safari in front.
...
- **Starter set:** `apps/server/seed/questions.json` holds the hand-written multiple-choice questions, and `bluff.json`, `timeline.json` and `numbers.json` the party modes' (Blöffölő, Időrend, Tippelj!). Every deploy loads any new ones and skips ones already in the database.

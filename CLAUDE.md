...
- `apps/motion` holds the paper-studio redesign (not yet used by the game): the art generators in `apps/motion/art` and the Remotion clips in `apps/motion/src`. `pnpm motion:art` builds the drawings, `pnpm motion:studio` previews the clips, `pnpm motion:render` renders the show open; see its README.
